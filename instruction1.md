<context>
Currently, the jwt.guard file strictly enforces whether or not we include a bearer token in all requests. Well, I want it so that when NODE_ENV=Development, we don't need to use JWT. As far as I remember, we just need to comment out or add a comment to this line in jwt.guard `const accessUser = await this.authService.verifyAccessToken(token, lang);`. However, I want the solution to be more robust and reliable.
