...
import { IOwner, IRole, IOwnerWithRoles, IUserRow } from '../interfaces';

/**
 * Owner Registration Repository
 *
 * Handles data access operations for owner registration.
 * Manages PII encryption/decryption transparently.
 *
 * @class OwnerRegistrationRepository
 * @version 1.0.0
 * @since 2026-01-29
 */
@Injectable()
export class OwnerRegistrationRepository {
    ...
    for (const user of users) {
      ...
      try {
        ...
        if (decryptedEmail.toLowerCase() === email.toLowerCase()) {
          return this.mapRowToOwner(user);
        }
      } catch {
   ...
   */
  async findByIdWithRoles(userId: string): Promise<IOwnerWithRoles | null> {
  ...
  private decryptUser(
    user: IUserRow,
  ): Omit<IOwner, 'password' | 'is_encrypted'> {
    let name = user.name;
  ...
  /**
   * Map database row to owner interface
   */
  private mapRowToOwner(row: IUserRow): IOwner {
    return {
      id: row.id,
