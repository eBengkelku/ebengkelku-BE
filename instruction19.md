...
**Description:**
Endpoint untuk manage master data product types (e.g., "Tools", "Spare Parts"). Data ini dipakai sebagai parent dari product categories.

**Acceptance Criteria:**

- [ ] `POST /v1/product-types` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Required: `name`
  - Auto-populate: `id`, `created_at`, `id_creator`
  - Validasi: `name` harus unique
- [ ] `GET /v1/product-types` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Support pagination (`page`, `limit`)
  - Return: `{ data: [], meta: { total, page, limit } }`
- [ ] `GET /v1/product-types/:id` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Return single product type detail
- [ ] `PUT /v1/product-types/:id` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Update `name`, `updated_at`, `id_updater`
  - Validasi: `name` harus unique (kecuali nama sendiri)
- [ ] `DELETE /v1/product-types/:id` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Soft delete (set `deleted_at`)
  - Validasi: tidak bisa delete jika masih ada categories yang menggunakan
- [ ] Error handling untuk constraint violations

---

**Description:**
Endpoint untuk manage product categories. Setiap category terkait dengan satu product type.

**Acceptance Criteria:**

- [ ] `POST /v1/product-categories` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Required: `name`, `product_type_id`
  - Optional: `description`
  - Auto-populate: `id`, `created_at`, `id_creator`
  - Validasi: `product_type_id` harus valid dan aktif (tidak soft deleted)
- [ ] `GET /v1/product-categories` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Support pagination (`page`, `limit`)
  - Support filter by `product_type_id`
  - Return: `{ data: [], meta: { total, page, limit } }`
  - Include product type info di response
- [ ] `GET /v1/product-categories/:id` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Return single category detail with product type info
- [ ] `PUT /v1/product-categories/:id` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Update `name`, `description`, `product_type_id`, `updated_at`, `id_updater`
  - Validasi: `product_type_id` harus valid dan aktif
- [ ] `DELETE /v1/product-categories/:id` — **owner atau orang-orang yang terasosiasi dengan business only**
  - Soft delete (set `deleted_at`)
  - Validasi: tidak bisa delete jika masih ada products yang menggunakan
- [ ] Error handling untuk foreign key violations
