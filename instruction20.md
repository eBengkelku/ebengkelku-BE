...
**Description:**
Endpoint CRUD untuk manage products di bengkel tertentu. Hanya owner dari business yang bisa akses.

**Acceptance Criteria:**

- [ ] `POST /v1/products/:businessId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Required: `name`, `price`, `category_id`
  - Optional: `description`, `unit` (default: 'pcs')
  - Auto-populate: `id`, `business_id` (from URL param), `status` (default: 'active'), `created_at`, `id_creator`
  - Validasi:
    - User adalah owner dari business tersebut
    - `category_id` harus valid dan aktif (tidak soft deleted)
    - `business_id` dari URL harus valid dan active
    - `price` harus positive integer
- [ ] `GET /v1/products/:businessId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Support pagination (`page`, `limit`)
  - Support filter by `status` (active/draft/archived)
  - Support filter by `category_id`
  - Return: `{ data: [], meta: { total, page, limit } }`
  - Include category info di response
  - Hanya tampilkan products yang belum soft deleted
- [ ] `GET /v1/products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Return single product detail with category info
  - Validasi: product harus milik business tersebut
- [ ] `PUT /v1/products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Update: `name`, `description`, `price`, `unit`, `status`, `category_id`, `updated_at`, `id_updater`
  - **TIDAK boleh** update `business_id`
  - Validasi:
    - User adalah owner dari business tersebut
    - Product harus milik business tersebut
    - `category_id` (jika diupdate) harus valid dan aktif
    - `status` harus salah satu dari: active/draft/archived
    - `price` harus positive integer
- [ ] `DELETE /v1/products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Soft delete (set `deleted_at`)
  - Validasi:
    - User adalah owner dari business tersebut
    - Product harus milik business tersebut
  - Cascade soft delete ke `tool_products`, `spare_part_products`, dan `inventories` (handled by DB constraint)
