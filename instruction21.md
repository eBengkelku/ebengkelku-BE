...
**Description:**
Endpoint untuk manage detail khusus tool products (warranty info). Extend dari base products.

**Acceptance Criteria:**

- [ ] `POST /v1/tool-products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Required: `product_id` (dari URL)
  - Optional: `warranty_months` (default: 0)
  - Auto-populate: `id_creator`
  - Validasi:
    - User adalah owner dari business tersebut
    - `product_id` harus valid dan milik business tersebut
    - Product belum punya record di `tool_products` (unique constraint)
    - `warranty_months` harus non-negative integer
- [ ] `GET /v1/tool-products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Return tool product detail
  - Include base product info
  - Validasi: product harus milik business tersebut
- [ ] `PUT /v1/tool-products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Update: `warranty_months`, `updated_at`, `id_updater`
  - Validasi:
    - User adalah owner dari business tersebut
    - Product harus milik business tersebut
    - `warranty_months` harus non-negative integer
- [ ] `DELETE /v1/tool-products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Soft delete (set `deleted_at`)
  - Validasi: product harus milik business tersebut
  - **Note:** tidak delete base product, hanya tool-specific data
- [ ] Authorization middleware: verify user is owner of the business
- [ ] Error handling untuk constraint violations

---

**Description:**
Endpoint untuk manage detail khusus spare part products (brand, grade). Extend dari base products.

**Acceptance Criteria:**

- [ ] `POST /v1/spare-part-products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Required: `product_id` (dari URL)
  - Optional: `brand`, `grade` (genuine/aftermarket)
  - Auto-populate: `id_creator`
  - Validasi:
    - User adalah owner dari business tersebut
    - `product_id` harus valid dan milik business tersebut
    - Product belum punya record di `spare_part_products` (unique constraint)
    - `grade` harus salah satu dari: genuine/aftermarket (jika diisi)
- [ ] `GET /v1/spare-part-products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Return spare part product detail
  - Include base product info
  - Validasi: product harus milik business tersebut
- [ ] `PUT /v1/spare-part-products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Update: `brand`, `grade`, `updated_at`, `id_updater`
  - Validasi:
    - User adalah owner dari business tersebut
    - Product harus milik business tersebut
    - `grade` harus salah satu dari: genuine/aftermarket (jika diisi)
- [ ] `DELETE /v1/spare-part-products/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Soft delete (set `deleted_at`)
  - Validasi: product harus milik business tersebut
  - **Note:** tidak delete base product, hanya spare-part-specific data
- [ ] Authorization middleware: verify user is owner of the business
- [ ] Error handling untuk constraint violations
...
**Description:**
Endpoint untuk manage inventory/stock products di bengkel. Tracking quantity dan min stock level.

**Acceptance Criteria:**

- [ ] `POST /v1/inventories/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Required: `product_id` (dari URL)
  - Optional: `quantity` (default: 0), `min_stock` (default: 0)
  - Auto-populate: `id`, `id_creator`
  - Validasi:
    - User adalah owner dari business tersebut
    - `product_id` harus valid dan milik business tersebut
    - Product belum punya record di `inventories` (one-to-one)
    - `quantity` dan `min_stock` harus non-negative integer
- [ ] `GET /v1/inventories/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Return inventory detail for specific product
  - Include product info (name, unit)
  - Validasi: product harus milik business tersebut
- [ ] `GET /v1/inventories/:businessId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - List all inventories for business
  - Support pagination (`page`, `limit`)
  - Include product basic info
  - Optional: filter by low stock (`quantity < min_stock`)
  - Return: `{ data: [], meta: { total, page, limit } }`
- [ ] `PUT /v1/inventories/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Update: `quantity`, `min_stock`, `updated_at`, `id_updater`
  - Validasi:
    - User adalah owner dari business tersebut
    - Product harus milik business tersebut
    - `quantity` dan `min_stock` harus non-negative integer
- [ ] `DELETE /v1/inventories/:businessId/:productId` — **business owner atau orang-orang yang terasosiasi dengan business only**
  - Soft delete (set `deleted_at`)
  - Validasi: product harus milik business tersebut
- [ ] Authorization middleware: verify user is owner of the business
