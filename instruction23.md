<context>
Kita akan membuat endpoint untuk get all semua data tool products dengan endpoint `GET /v1/tool-products/{businessId}`. Kemudian kita juga akan membuat endpoint untuk get all semua data spare part products dengan endpoint `GET /v1/spare-part-products/{businessId}`.

Hal ini akan digunakan untuk halaman inventory di frontend, jadi saya ingin ketika memanggil endpoint tersebut, data yang dikembalikan adalah data yang sudah di-join dengan tabel products, sehingga saya bisa mendapatkan data nama, deskripsi, harga, unit, status, category_id, business_id, id, dan lain-lain. Intinya semua informasi yang berkaitan dengan product itu juga harus muncul. Tapi, harus sesuai dengan endpointnya, kalau tool-products berarti yang relate dengan tool-products, begitu juga dengan spare part.
